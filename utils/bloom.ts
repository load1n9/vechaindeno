...
export class Bloom {
     ...
     */
    constructor(k: number, bits?: typeof Buffer) {
        if (bits) {
            this.bits = bits
        } else {
     ...
     * @param item
     */
    public test(item: typeof Buffer) {
    ...
    private distribute(item: typeof Buffer, cb: (index: number, bit: number) => boolean): boolean {
